...
trait InstallHelperTrait
{
				...
				if ($oldValue !== '')
				{
					...
					if ($newValue !== false || $oldValue === 'b:0;')
					{
						...
						if ($encodedValue === false && $newValue !== false)
						{
							...
							\XF::logError(
								"Error doing JSON conversion $tableName.$column ($pk=$pkValue): " . json_last_error_msg(),
								true
							);
						}
						else
	...
	public function renameLikePermissionsToReactions(array $permissionGroupIds, $likePermissionId = 'like')
	{
		$globalPermissions = [];
		$contentPermissions = [];
