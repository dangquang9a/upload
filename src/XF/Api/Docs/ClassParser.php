...
class ClassParser
{
	...
	protected function getTypeNameFromShortName($shortName)
	{
		...
		{
			return str_replace($nameParts[0], '\\', '_') . '_' . $nameParts[1];
		}
	}
	...
	public function parseClassMethod($commentType, $className, $method)
	{
		...
		switch ($commentType)
		{
			...
			default:
				throw new \LogicException("Unknown block type expected");
		}
	}
	...
	{
		return $this->annotationParser->parse(AnnotationParser::BLOCK_TYPE, $comment);
	}
	...
	{
		return $this->annotationParser->parse(AnnotationParser::BLOCK_ROUTE, $comment);
	}
}