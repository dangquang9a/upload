...
class Mail
{
	...
	public function setTo($email, $name = null)
	{
		...
		$this->applyVerp($this->bounceHmac, $this->bounceVerpBase);
	...
	public function setReturnPath($email, $useVerp = false)
	{
		...
		if ($useVerp)
		{
			...
			$verpAddress = $this->applyVerp($this->bounceHmac, $this->bounceVerpBase);

			if (is_string($verpAddress) && $verpAddress !== $email)
			{
				try
				{
	...
	public function setListUnsubscribe($unsubEmail, $useVerp = false)
	{
		...
		$hmac = $this->toUser->getEmailConfirmKey();
		$userEmail = $this->toUser->email;

		if ($useVerp)
		{
			$verpAddress = $this->applyVerp($this->toUser->getEmailConfirmKey(), $unsubEmail);

			if (is_string($verpAddress) && $verpAddress !== $unsubEmail)
			{
	...
	protected function applyVerp($hmac, $verpBase)
	{
		if (!$hmac || !$verpBase)
		{
			return $this;
