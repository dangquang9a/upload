...
!function($, window, document)
{
	...
	XF.Feature.runTest('hiddenscroll', function () {
		// todo: move this to preamble.js (but needs to be able to function without jQuery and a body)
		var $el = $('<div />').css({
				width: '100px',
				height: '100px',
				overflow: 'scroll',
		...
		var result = (el.offsetWidth === el.clientWidth);
	...
	XF.AutoCompleteHandler = XF.create({
		options: {},
		config: {},
		...
		__construct: function($el, options, config)
		{
			var t = this;
			...
			this.options = $.extend({
				...
				defaultCallbacks: {
					remoteFilter: function(query, callback)
					{
						...
						if (!query || query.length < t.options.minQueryLength)
						{
							return;
						}

						XF.ajax(
							t.options.method, t.options.url, { q: query },
							function(data)
							{
					...
					{
						XF.setRelativeZIndex(this.$el.find('.atwho-view'), this.$inputor, 1);
					}
				},
				customCallbacks: {}
			}, options);
			...
			this.config = $.extend({
				...
				searchKey: 'text',
				headerTpl: '',
				displayTpl: '<li>' + (this.options.includeIcon ? '<span class="autoCompleteList-icon">${iconHtml}</span>' : '') + '${text}</li>',
				insertTpl: (this.options.includeAt ? '${atwho-at}' : '') + '${text}',
				callbacks: {}
			}, config);

			this.config.callbacks = $.extend(this.options.defaultCallbacks, this.config.callbacks);
		...
		{
			var $el = this.$el,
				ed = this.options.editor;

			this.at = $el.atwho(this.config);

			// if we don't do this, the atwho-view objects may be visible before use
			$('.atwho-container .atwho-view:not([data-initialized])')
				.attr('data-initialized', '1')
				.css('display', 'none');

			if (this.options.customCallbacks.insert)
			{
				this.options.customCallbacks.insert.call(this, $el, ed);
			}
			...
			if (ed)
			{
				...
				{
					if (e.which === $.FroalaEditor.KEYCODE.ENTER && $el.atwho('isSelecting'))
					{
						return false;
					}
				}, true);
