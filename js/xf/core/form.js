...
!function($, window, document)
{
	...
	XF.UserMentioner = XF.Element.newHandler({
		...
		init: function()
		{
			...
			this.handler = new XF.AutoCompleteHandler(
				this.$target, handlerOpts
			);
		}
	});
	...
	XF.EmojiCompleter = XF.Element.newHandler({
		...
		init: function()
		{
			...
			var atJsConfig = {
				at: ':',
				insertTpl: this.options.insertTemplate,
				callbacks: {
			...
			this.handler = new XF.AutoCompleteHandler(
				this.$target, emojiHandlerOpts, atJsConfig
			);
		}
	});
	...
	XF.SelectPlus = XF.Element.newHandler(
	{
		...
		setActive: function(forceActive)
		{
			...
			if (this.isActive != previouslyActive)
			{
				...
				$(document.body).toggleClassTransitioned('SelectPlusActive', this.isActive);
			}
		},
