!function($, window, document, _undefined)
{
	...
	XF.Editor = XF.Element.newHandler({
		...
		init: function()
		{
			...
			this.$placeholder = this.$target.closest('.js-editorPlaceholder');

			if (this.options.deferred && this.$placeholder.length)
			{
				this.$placeholder.on('click', XF.proxy(this, 'startInit'));
			}
			else
			{
				this.startInit();
		...
		startInit: function(e, clickInit)
		{
			var t = this,
				$placeholder = this.$placeholder,
				eventTriggered = (typeof e !== 'undefined');
			...
			this.$target
				...
				.on('froalaEditor.initialized', function (m, ed)
				{
					...
					if (eventTriggered)
					{
						$placeholder.find('.editorPlaceholder-editor').removeClass('is-hidden');
						$placeholder.find('.editorPlaceholder-placeholder').addClass('is-hidden');
					}
					...
					if (eventTriggered)
					{
						...
						{
							t.scrollToCursor();
							t.scrollToCursorAfterPendingResize();
						}
		...
		getButtonConfig: function()
		{
			...
			var buttonClass = this.getButtonClasses();

			var removeButton = function(buttons, removeNames)
			{
				if (typeof removeNames == 'string' && buttonClass[removeNames])
				{
					removeNames = buttonClass[removeNames];
				}
				...
				{
					return !(removeNames.indexOf(button) >= 0);
				});
			};

			var removeButtonsFromToolbars = function(toolbars, removeNames)
			{
				for (var key in editorToolbars)
				{
					if (editorToolbars.hasOwnProperty(key))
					{
						editorToolbars[key] = removeButton(editorToolbars[key], removeNames);
					}
				}
			};
			...
			{
				removeButtonsFromToolbars(editorToolbars, 'xfDraft');
			}

			var attachmentManager = this.getAttachmentManager();
			if (!attachmentManager || !attachmentManager.supportsVideoUploads)
			{
				removeButtonsFromToolbars(editorToolbars, 'insertVideo');
			}

			if (this.options.buttonsRemove)
			{
				var i, remove = this.options.buttonsRemove.split(',');
				for (i = 0; i < remove.length; i++)
				{
					removeButtonsFromToolbars(editorToolbars, remove[i]);
				}
			}

			var convertAllToolbarSeparators = function(toolbars)
			{
				for (var key in editorToolbars)
				{
					if (editorToolbars.hasOwnProperty(key))
					{
						editorToolbars[key] = convertSeparators(editorToolbars[key]);
					}
				}
			...
			var convertSeparators = function(config)
			{
			...
			editorToolbars = convertAllToolbarSeparators(editorToolbars);

			var eventData = {
				editorButtons: editorToolbars,
				helpers: {
					removeButton: removeButton,
					removeButtonsFromToolbars: removeButtonsFromToolbars
				}
			};
		...
		editorInit: function()
		{
			...
			var mentionerHandlerOpts = {
				url: XF.getAutoCompleteUrl(),
				editor: ed
			};
			this.mentioner = new XF.AutoCompleteHandler(
				ed.$el, mentionerHandlerOpts
			);
			...
			if (XF.config.shortcodeToEmoji)
			{
				var emojiHandlerOpts = {
					...
					customCallbacks: {
						insert: function($el, ed)
						{
							...
							var t = this;
							ed.$el.on('inserted.atwho', function()
							{
								var $inserted = ed.$el.find('.atwho-inserted').hide(),
									$inner = $inserted.find('img.smilie').length
										? $inserted.find('img.smilie') // img
										: $inserted.contents(); // native emoji

								$inserted.replaceWith($inner);
							});
						}
					}
				};
				var atJsConfig = {
					at: ':',
					searchKey: 'text_desc', // custom value returned from the view
					displayTpl: '<li>' +
						'<div class="contentRow">' +
						'<div class="contentRow-figure contentRow-figure--emoji">${iconHtml}</div>' +
					...
					callbacks: {
						...
						{
							XF.logRecentEmojiUsage($li.find('img.smilie').data('shortname'));

							return value;
						}
					}
				};
				this.emojiCompleter = new XF.AutoCompleteHandler(
					ed.$el, emojiHandlerOpts, atJsConfig
				);
			}
